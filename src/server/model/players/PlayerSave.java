...
public class PlayerSave {
	...
	/**
	 * The {@link Logger} instance for {@link PlayerSave}
	 */
	private static Logger logger = Logger.getLogger(PlayerSave.class.getName());
	...
	public static int loadGame(Client p, String playerName, String playerPass) {
		...
		if (File1) {
			...
			logger.info(playerName + " character file not found.");
			p.newPlayer = false;
			return 0;
		}
		try {
			...
			logger.info(playerName + " error loading save.");
			return 3;
		}
		while (EndOfFile == false && line != null) {
			...
			if (spot > -1) {
				...
				case 2:
					...
					} else if (token.equals("demonSlayer")) {
						p.demonSlayer = Integer.parseInt(token2);
					...
					} else if (token.equals("boneSearch")) {
						p.boneSearch = Boolean.parseBoolean(token2);
					} else if (token.equals("captainRovin")) {
						p.captainRovin = Boolean.parseBoolean(token2);
					} else if (token.equals("prysin")) {
						p.prysin = Boolean.parseBoolean(token2);
					} else if (token.equals("trailborn")) {
						p.trailborn = Boolean.parseBoolean(token2);
	...
	public static boolean saveGame(Client p) {
		...
		try {
			...
			characterfile.write(Integer.toString(p.doricQuest), 0, Integer
					.toString(p.doricQuest).length());
			characterfile.newLine();
			//
			...
			characterfile.write(Integer.toString(p.demonSlayer), 0, Integer
					.toString(p.demonSlayer).length());
			characterfile.newLine();
			characterfile.write("Incantation = ", 0, 14);
			characterfile.write(Integer.toString(p.Incantation), 0, Integer
					.toString(p.Incantation).length());
			characterfile.newLine();
			characterfile.write("boneSearch = ", 0, 10);
			characterfile.write(Boolean.toString(p.boneSearch), 0, Boolean
					.toString(p.boneSearch).length());

			characterfile.newLine();
			characterfile.write("captainRovin = ", 0, 10);
			characterfile.write(Boolean.toString(p.captainRovin), 0, Boolean
					.toString(p.captainRovin).length());
			characterfile.newLine();
			characterfile.write("trailborn = ", 0, 10);
			characterfile.write(Boolean.toString(p.trailborn), 0, Boolean
					.toString(p.trailborn).length());
			characterfile.newLine();
			// endo quests
			...
			logger.info(p.playerName + " error writing player save.");
			return false;
		}
		return true;
	}
