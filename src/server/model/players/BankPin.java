...
public class BankPin {
    ...
    public void pinSettingFrames() {
      int pinSendFrames[] = {15075, 15080, 15110, 15171, 15076, 15176, 15104,15082, 15079};
        for (int j = 0; j < 9; j++) {
            client.getPA().sendFrame126("", pinSendFrames[j]);
        }
        client.getPA().sendFrame126("Welcome to our bank", 15038);
        client.getPA().sendFrame126("recovery system.", 15039);
        client.getPA().sendFrame126("Remember, it's important", 15040);
        client.getPA().sendFrame126("to change your recovery", 15041);
        client.getPA().sendFrame126("pin and password", 15042);
        client.getPA().sendFrame126("every 1-3 months", 15043);
        if (!client.hasBankpin) {
            client.getPA().sendFrame126("Set a Bank Pin", 15078);
            client.getPA().sendFrame126("No PIN Set", 15105);
        } else {
            client.getPA().sendFrame126("Delete your PIN", 15078);
            if (client.requestPinDelete)
                client.getPA().sendFrame126("Pending delete", 15105);
            else
                client.getPA().sendFrame126("Has Bank PIN", 15105);
        }
        client.getPA().sendFrame126(recovery_Delay+" days", 15107);
    }
    ...
    private int resetBankNumbers() {
        return firstPin = secondPin = thirdPin = fourthPin = client.playerBankPin =
                client.firstPin = client.secondPin = client.thirdPin = client.fourthPin = -1;
    }
    ...
    private void handleButtonFour(int button) {
        ...
        if (!client.hasBankpin) {
            ...
            client.sendMessage("Your new Bank PIN is: " + firstPin + " - " + secondPin + " - " + thirdPin + " - " + fourthPin);
