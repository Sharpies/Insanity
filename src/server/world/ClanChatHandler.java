...
public class ClanChatHandler {
	...
	public boolean isOwner(Client c) {
		//String name = Server.playerHandler.players[c.playerId].playerName.toLowerCase();
			...
			if (Server.playerHandler.players[i] != null) {
				if (Server.playerHandler.players[i].playerName.equalsIgnoreCase(name)) {
					...
					if(c2.playerRights == 7){
						c.sendMessage("You may @red@NOT@bla@ kick the developer from you clan!");
...
String name = Server.playerHandler.players[playerId].playerName.toLowerCase();
name = name.substring(0, 1).toUpperCase()  + name.substring(1);
messageToClan(name + " has joined the channel.", clanId);					
...
for (int j = 0; j < clans[clanId].members.length; j++) {
	int playerIdSlot = clans[clanId].members[j];

	if(playerIdSlot > -1) {
		Client player = (Client)Server.playerHandler.players[playerIdSlot];
	
		if(player != null) {
		String name = Server.playerHandler.players[playerId].playerName.toLowerCase();
		name = name.substring(0, 1).toUpperCase()  + name.substring(1);
			player.sendMessage(name + " has left the clan.");
		}
	}
}
			}
	...
	public void playerMessageToClan(int playerId, String message, int clanId) {
		...
		for (int j = 0; j < clans[clanId].members.length; j++) {
			...
			if (Server.playerHandler.players[clans[clanId].members[j]] != null) {
				...
				c.sendClan(Misc.optimizeText(Server.playerHandler.players[playerId].playerName), message, Misc.optimizeText(clans[clanId].name), Server.playerHandler.players[playerId].playerRights);
			}
		}	
	}
	...
	public void handleLootShare(Client c, int item, int amount) {
		//String name2 = Server.playerHandler.players[playerId].playerName.toLowerCase();
		//c.playerName.substring(0, 1).toUpperCase()  + c.playerName.substring(1);
