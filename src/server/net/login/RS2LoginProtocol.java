...
public class RS2LoginProtocol extends FrameDecoder {
	...
	/**
	 * The {@link Logger} instance for {@link RS2LoginProtocol}
	 */
	private static Logger logger = Logger.getLogger(RS2LoginProtocol.class.getName());
	...
	protected Object decode(ChannelHandlerContext ctx, Channel channel, ChannelBuffer buffer) throws Exception {
		...
		case CONNECTED:
			...
			return null;
		case LOGGING_IN:
			logger.info("A connection has been received from: [" + channel + "].");
			if (buffer.readableBytes() < 2) {
				return null;
			}
			...
			if (clientVersion != 317) {
				logger.info("Invalid client revision " + clientVersion);
				channel.close();
				return null;
			}
			buffer.readByte();
